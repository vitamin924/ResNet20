...
Ctxt layer3(const Ctxt& in) {
    ...
    vector<Ctxt> res1sx = controller.convbn3264sx(boot_in, 11, 1, scaleSx, timing); //Questo è lento
    vector<Ctxt> res1dx = controller.convbn3264dx(boot_in, 11, 1, scaleDx, timing); //Questo è lento
    ...
    fullpackSx = controller.convbn3(fullpackSx, 11, 2, scaleDx, timing);
    ...
    start = start_time();
    Ctxt res2;
    res2 = controller.convbn3(res1, 12, 1, scale, timing);
    res2 = controller.bootstrap(res2, timing);
    res2 = controller.relu(res2, scale, timing);

    scale = 0.37;

    res2 = controller.convbn3(res2, 12, 2, scale, timing);
    ...
    res3 = controller.convbn3(res2, 13, 1, scale, timing);
    res3 = controller.bootstrap(res3, timing);
    res3 = controller.relu(res3, scale, timing);

    scale = 0.32;

    res3 = controller.convbn3(res3, 13, 2, scale, timing);
...
// Block 4
    scale = 0.49;
    if (verbose > 1) cout << "---Start: Layer3 - Block 4---" << endl;
    start = start_time();
    Ctxt res4;

    res4 = controller.convbn3(res3, 14, 1, scale, timing);
    res4 = controller.bootstrap(res4, timing);
    res4 = controller.relu(res4, scale, timing);

    scale = 0.17;

    res4 = controller.convbn3(res4, 14, 2, scale, timing);
    res4 = controller.add(res4, controller.mult(res3, scale));
    res4 = controller.bootstrap(res4, timing);
    res4 = controller.relu(res4, scale, timing);
    res4 = controller.bootstrap(res4, timing);

    if (verbose > 1) print_duration(start, "Total");
    if (verbose > 1) cout << "---End  : Layer3 - Block 4---" << endl;

    // Block 5
    scale = 0.85;
    if (verbose > 1) cout << "---Start: Layer3 - Block 5---" << endl;
    start = start_time();
    Ctxt res5;

    res5 = controller.convbn3(res4, 15, 1, scale, timing);
    res5 = controller.bootstrap(res5, timing);
    res5 = controller.relu(res5, scale, timing);

    scale = 0.10;

    res5 = controller.convbn3(res5, 15, 2, scale, timing);
    res5 = controller.add(res5, controller.mult(res4, scale));
    res5 = controller.bootstrap(res5, timing);
    res5 = controller.relu(res5, scale, timing);
    res5 = controller.bootstrap(res5, timing);

    if (verbose > 1) print_duration(start, "Total");
    if (verbose > 1) cout << "---End  : Layer3 - Block 5---" << endl;
...
Ctxt layer2(const Ctxt& in) {
    ...
    vector<Ctxt> res1sx = controller.convbn1632sx(boot_in, 6, 1, scaleSx, timing); //Questo è lento

    vector<Ctxt> res1dx = controller.convbn1632dx(boot_in, 6, 1, scaleDx, timing); //Questo è lento
    ...
    fullpackSx = controller.convbn2(fullpackSx, 6, 2, scaleDx, timing);
    Ctxt res1 = controller.add(fullpackSx, fullpackDx);
    res1 = controller.bootstrap(res1, timing);
    res1 = controller.relu(res1, scaleDx, timing);
    if (verbose > 1) print_duration(start, "Total");
    ...
    start = start_time();
    Ctxt res2;
    res2 = controller.convbn2(res1, 7, 1, scale, timing);
    res2 = controller.bootstrap(res2, timing);
    res2 = controller.relu(res2, scale, timing);

    scale = 0.33;

    res2 = controller.convbn2(res2, 7, 2, scale, timing);
    res2 = controller.add(res2, controller.mult(res1, scale));
    res2 = controller.bootstrap(res2, timing);
    res2 = controller.relu(res2, scale, timing);
    if (verbose > 1) print_duration(start, "Total");
    ...
    start = start_time();
    Ctxt res3;
    res3 = controller.convbn2(res2, 8, 1, scale, timing);
    res3 = controller.bootstrap(res3, timing);
    res3 = controller.relu(res3, scale, timing);
  
    scale = 0.34;

    res3 = controller.convbn2(res3, 8, 2, scale, timing);
    res3 = controller.add(res3, controller.mult(res2, scale));
    res3 = controller.bootstrap(res3, timing);
    res3 = controller.relu(res3, scale, timing);
    if (verbose > 1) print_duration(start, "Total");
    ...
    // Block 4
    scale = 0.83;
    if (verbose > 1) cout << "---Start: Layer2 - Block 4---" << endl;
    start = start_time();
    Ctxt res4;
    res4 = controller.convbn2(res3, 9, 1, scale, timing);
    res4 = controller.bootstrap(res4, timing);
    res4 = controller.relu(res4, scale, timing);

    scale = 0.24;

    res4 = controller.convbn2(res4, 9, 2, scale, timing);
    res4 = controller.add(res4, controller.mult(res3, scale));
    res4 = controller.bootstrap(res4, timing);
    res4 = controller.relu(res4, scale, timing);
    if (verbose > 1) print_duration(start, "Total");
    if (verbose > 1) cout << "---End  : Layer2 - Block 4---" << endl;

    // Block 5
    scale = 0.83;
    if (verbose > 1) cout << "---Start: Layer2 - Block 5---" << endl;
    start = start_time();
    Ctxt res5;
    res5 = controller.convbn2(res4, 10, 1, scale, timing);
    res5 = controller.bootstrap(res5, timing);
    res5 = controller.relu(res5, scale, timing);

    scale = 0.25;

    res5 = controller.convbn2(res5, 10, 2, scale, timing);
    res5 = controller.add(res5, controller.mult(res4, scale));
    res5 = controller.bootstrap(res5, timing);
    res5 = controller.relu(res5, scale, timing);
    if (verbose > 1) print_duration(start, "Total");
    if (verbose > 1) cout << "---End  : Layer2 - Block 5---" << endl;
...
scale = 0.81;
    if (verbose > 1) cout << "---Start: Layer1 - Block 4---" << endl;
    start = start_time();
    Ctxt res4;
    res4 = controller.convbn(res3, 4, 1, scale, timing);
    res4 = controller.bootstrap(res4, timing);
    res4 = controller.relu(res4, scale, timing);

    scale = 0.41;

    res4 = controller.convbn(res4, 4, 2, scale, timing);
    res4 = controller.add(res4, controller.mult(res3, scale));
    res4 = controller.bootstrap(res4, timing);
    res4 = controller.relu(res4, scale, timing);
    if (verbose > 1) print_duration(start, "Total");
    if (verbose > 1) cout << "---End  : Layer1 - Block 4---" << endl;

    // Block 5
    scale = 0.72;
    if (verbose > 1) cout << "---Start: Layer1 - Block 5---" << endl;
    start = start_time();
    Ctxt res5;
    res5 = controller.convbn(res4, 5, 1, scale, timing);
    res5 = controller.bootstrap(res5, timing);
    res5 = controller.relu(res5, scale, timing);

    scale = 0.35;

    res5 = controller.convbn(res5, 5, 2, scale, timing);
    res5 = controller.add(res5, controller.mult(res4, scale));
    res5 = controller.bootstrap(res5, timing);
    res5 = controller.relu(res5, scale, timing);
    if (verbose > 1) print_duration(start, "Total");
    if (verbose > 1) cout << "---End  : Layer1 - Block 5---" << endl;
